...
While there are currently no built-in functions that allow for full customization 
...
For example, if you want to change the background-color of the Review box heading,
simply select the `#jkl_review_box_head` in your Custom CSS editor in WordPress 
and change the `background-color` property there.
...
the preview of your Cover Image in the Custom Meta Box, you need to first either
`Save Draft` or `Publish` or `Update` your Post so that it **saves** the meta data first.
...
Currently there is no way to add multiples of anything. 
* If you want to add multiple authors, series, or categories, just type in the two (or 
more) names in the appropriate fields.
* If you want to add multiple images, use the Post Editor to do so.
* If you want to add multiple links, add them in the Post Editor.
...
I can take a look at it. Things to include in your message include:
* Your WordPress installation version number
* The Theme you are using (and its version number - if relevant)
